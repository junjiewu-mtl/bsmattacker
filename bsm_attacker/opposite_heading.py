"""
Opposite Heading Attack (VASP heading/Opposite)
================================================

The attacker reports the heading rotated by 180 degrees from the true
heading, making it appear the vehicle is traveling in the opposite
direction.

Attack Mechanism:
- Target: Selected messages from all vehicles
- Modification: h' = (h + 180) % 360
- Goal: Mislead IMA and collision avoidance by reversing apparent direction

Literature Reference:
- VASP: heading/Opposite
...
class OppositeHeadingAttacker(BaseAttacker):
    """
    Implements the 'Opposite Heading' attack (VASP heading/Opposite).

    Attacked messages report a heading rotated 180 degrees from the true
    heading, creating the illusion of opposite-direction travel.
    """

    def __init__(self, random_seed: int = 42):
        super().__init__(attack_name="Opposite Heading", random_seed=random_seed)

    def inject_attack(self, df: pd.DataFrame,
                      attack_ratio: float = 0.15,
                      target_vehicles: list = None) -> pd.DataFrame:
        """
        Inject 'Opposite Heading' attacks into BSM data.
        ...
        for vehicle_id in target_vehicles:
            ...
            df.loc[attack_indices, heading_col] = (
                df.loc[attack_indices, heading_col] + 180
            ) % 360
